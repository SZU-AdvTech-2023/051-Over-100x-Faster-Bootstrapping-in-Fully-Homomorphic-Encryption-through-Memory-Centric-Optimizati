...
auto genBitReversedTwiddleFacotrs(const word64 root, const word64 p,
  ...
  const auto root_inverse = Inverse(root, p); //w-1
  pow_roots[0] = 1;
  inv_pow_roots[0] = 1;
  for (int i = 1; i < degree; i++) {
    pow_roots[i] = MulMod(pow_roots[i - 1], root, p);//1 w w2 w3 w4
    inv_pow_roots[i] = MulMod(inv_pow_roots[i - 1], root_inverse, p);//1 w-1 w-2 w-3
  }
...
Context::Context(const Parameter &param)
  ...
  for (auto p : param.primes_) {
    ...
    auto root = findPrimitiveRoot(p);//w
    root = powMod(root, (p - 1) / (2 * degree__), p);//w^((p-1)/2*N)mod p
...
//GPU kernel 
__global__ void rotate_(word64 *in ,word64 * out,word64*index,int m,int n){
    int row = blockIdx.x * blockDim.x + threadIdx.x;
    int col = blockIdx.y * blockDim.y + threadIdx.y;

    if (row < m && col < n) {
        int new_col = index[col];
        out[row * n + col] = in[row * n + new_col];
    }
}
//caculate 5^{-1}
...
//pi^{-1}
long long invers_pi(long long n, long long degree,long long x){
    long long inv_5n,inv_2n;
    long long a=pow(5,n),b=2*degree;
    extended_gcd(a,b,inv_5n,inv_2n);
    if(inv_5n<0){
        inv_5n +=  b;
        inv_2n -= a;
    }
    return ((inv_5n*(2*x+1))%(2*degree)-1)/2;
}
/*
in: input
sn: number of roations
out: output
d_index: out[index[i]]=in[i]
*/   
void Context::FrobeniusMap(DeviceVector &in,int sn,DeviceVector &out,DeviceVector &d_index) const{
  int m=param__.chain_length_,n=degree__;
  in.resize(m*n);
  out.resize(m*n);
  ToNTTInplace(in.data(),0,param__.chain_length_);

  dim3 blockDim(16, 16);
  dim3 gridDim((m + blockDim.x - 1) / blockDim.x, (n + blockDim.y - 1) / blockDim.y);
  rotate_<<<gridDim, blockDim>>>(in.data(), out.data(), d_index.data(), m, n);
  
  // HostVector test1=HostVector(d_index);
  // std::cout<<std::endl;
  // auto idx2=test1.data();
  // for(int i=0;i<10;i++){
  //     std::cout<<idx2[i]<<' ';
  // }
...
/*
ct: Input Ciphertext
evk: Used to run keyswitch
sn: number of rotations
out: Ciphertext after rotate
*/
void Context::rotate(const Ciphertext &ct, const EvaluationKey &evk,int sn,
                  Ciphertext &out) const {
  DeviceVector tmp_ax,tmp_bx;
  HostVector h_index;
  h_index.resize(degree__);

  auto idx=h_index.data();
  for(int i=0;i<degree__;i++){
    idx[i]=(word64)invers_pi(sn,degree__,i);//caculate pi^{-1}(x) store in h_index
  }

  DeviceVector d_index(h_index); // transfer data to gpu
 
  DeviceVector op_ax(ct.getAxDevice()) ;
  DeviceVector op_bx(ct.getBxDevice()) ;
  DeviceVector out_ax(out.getAxDevice()) ;
  DeviceVector out_bx(out.getBxDevice()) ;

  out_ax.resize(op_ax.size());
  out_bx.resize(op_ax.size());
  tmp_ax.resize(op_ax.size());
  tmp_bx.resize(op_ax.size());

//a'= FrobeniusMap(a,sn)
//b'= FrobeniusMap(b,sn)
  FrobeniusMap(op_ax,sn,tmp_ax,d_index);
  FrobeniusMap(op_bx,sn,tmp_bx,d_index);//tmp_bx b'
//(a'',b'')=KeySwitch(a',evk)
  KeySwitch(tmp_ax,evk,out_ax,out_bx);//out_bx b''

  const int length = op_ax.size() / degree__;
//b''=b'+b''
  int gridDim_ = 2048;
  int blockDim_ = 256;
  add_<<<gridDim_, blockDim_>>>(GetKernelParams(), length, out_bx.data(),
                                tmp_bx.data(), out_bx.data());
  CudaCheckError();
}